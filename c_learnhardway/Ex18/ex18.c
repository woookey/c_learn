...
int *insertion_sort(int *numbers, int count, compare_cb cmp)
{
	...
	for ( i = 1; i < count; i++)
	{
		for( k = i; k >= 1 && cmp(target[k], target[k-1]) > 0; k--)
		{
			temp = target[k];
			target[k] = target[k-1];
			target[k-1] = temp;
 ...
 */
void test_sorting(int *numbers, int count, compare_cb cmp, int* (*sort_f)(int *, int, compare_cb))
{
	int i = 0;
	int *sorted = sort_f(numbers, count, cmp);
